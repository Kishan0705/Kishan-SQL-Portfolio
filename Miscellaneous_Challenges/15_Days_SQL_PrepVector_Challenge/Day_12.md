...
## Day 12: User Consecutive Day Streak  
**Difficulty**: Hard  
...
> **Note**: A continuous streak means the user visited the platform at least once **every day** on **consecutive days**. Only streaks within the **year 2020** should be considered.
...
WITH filtered_events AS (
    SELECT 
        user_id, DATE(created_at) AS event_date
    FROM events 
...
steak_lengths AS (
    SELECT
        user_id, 
        base, 
        COUNT(*) AS steak_length 
    FROM grouped_streaks
    GROUP BY user_id, base
),
max_steaklength_per_user AS (
    SELECT 
        user_id, 
        MAX(steak_length) AS max_steak
    FROM steak_lengths 
    GROUP BY user_id
)
SELECT
    user_id, 
    max_steak
FROM max_steaklength_per_user
ORDER BY max_steak DESC, user_id
LIMIT 5;
```
